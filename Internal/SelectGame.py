...
from Internal.Util import fillGroupBox
...
def handleConnectorError(error):
    sessionTableWidget.setDisabled(True)
    sessionTableWidget.setLabelText(error)
...
def selectGame(loadGame):
    ...
    for file in glob.glob('*.py'):
        with open(file, 'r') as gameFile:
            for line in gameFile.readlines():
                if line.startswith('GamePlayer.run('):
